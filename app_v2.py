...
def search_similarities(query):
    ...
    if query and vector_db:
        ...
        st.chat_message("answer").markdown(response)
        # Add assistant response to chat history
        st.session_state.question_answer.append({"role": "answer", "content": response})
...
def question_answer():    
    ...
    if prompt := st.chat_input("Enter text here"):
        # Display user message in chat message container
        st.chat_message("question").markdown(prompt)
        # Add user message to chat history
        st.session_state.question_answer.append({"role": "question", "content": prompt})
        # search for similarities
        search_similarities(prompt)
